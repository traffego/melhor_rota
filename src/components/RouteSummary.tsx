...
💰 *CUSTO TOTAL:* ${formatCurrency(result.totalCost)}`;
  ...
  return (
    <div className="w-full space-y-3">
      ...
      <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-emerald-950 text-white p-3.5 sm:p-4 rounded-2xl shadow-xl space-y-3">
        <div className="flex items-center justify-between border-b border-white/10 pb-2.5">
          ...
          <div className="text-right">
            <span className="text-[11px] font-medium text-slate-300">Distância Total</span>
            <p className="text-base sm:text-lg font-bold text-emerald-300">{formatDistance(result.distanceKm)}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 text-xs">
          <div className="bg-white/5 backdrop-blur-sm p-2.5 rounded-xl border border-white/10">
            ...
            <p className="text-sm sm:text-base font-bold text-white">{formatCurrency(result.fuelCost)}</p>
          ...
          <div className="bg-white/5 backdrop-blur-sm p-2.5 rounded-xl border border-white/10">
            <div className="flex items-center gap-1 text-teal-400 font-semibold mb-0.5">
              <ReceiptText className="w-3.5 h-3.5" />
              <span>Pedágios</span>
            </div>
            <p className="text-sm sm:text-base font-bold text-white">{formatCurrency(result.totalTollCost)}</p>
            <p className="text-[10px] text-slate-300">
        ...
        <div className="flex items-center justify-between text-[11px] text-slate-300 pt-1 border-t border-white/10">
          <div className="flex items-center gap-1">
            ...
            <span>Tempo: <strong>{formatDuration(result.durationMinutes)}</strong></span>
          </div>
